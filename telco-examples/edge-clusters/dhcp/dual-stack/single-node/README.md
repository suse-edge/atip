...
- `bmh-example_v4dhcp-v6ra+dhcp_v4v6dns.yaml` - this configuration is identical to the `bmh-example_v4dhcp-v6ra+dhcp.yaml` above, with the exception of statically provided DNS servers; it can be useful in networks where only RAs are avaiable but with no DNS option.
     Requires:
...
- `single-cni`:
    - `cluster-v4v6-calico.yaml` - the cluster will use Calico as the sole CNI plugin in the cluster
    - `cluster-v4v6-canal.yaml` - the cluster will use Canal as the sole CNI plugin in the cluster
    - `cluster-v4v6-cilium.yaml` - the cluster will use Cilium as the sole CNI plugin in the cluster

- `multus-cni`:
   - `cluster-v4v6-calico_hostlocal.yaml` - the cluster will use Calico as the CNI plugin in combination with "host-local" as Multus IPAM for the second interface
   - `cluster-v4v6-calico_whereabouts.yaml` - the cluster will use Calico as the CNI plugin in combination with "whereabouts" as Multus IPAM for the second interface
   - `cluster-v4v6-canal_hostlocal.yaml` - the cluster will use Canal as the CNI plugin in combination with "host-local" as Multus IPAM for the second interface
   - `cluster-v4v6-canal_whereabouts.yaml` - the cluster will use Canal as the CNI plugin in combination with "whereabouts" as Multus IPAM for the second interface
   - `cluster-v4v6-cilium_hostlocal.yaml` - the cluster will use Cilium as the CNI plugin in combination with "host-local" as Multus IPAM for the second interface
   - `cluster-v4v6-cilium_whereabouts.yaml` - the cluster will use Cilium as the CNI plugin in combination with "whereabouts" as Multus IPAM for the second interface
