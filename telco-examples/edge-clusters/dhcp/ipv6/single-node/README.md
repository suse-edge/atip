...
- `bmh-example_slaac+dhcp.yaml` - both RAs and DHCP are enabled for autoconfiguration, resulting in a SLAAC deduced address and default route assignments, and DNS reception from DHCP (when not optionally provided by the RAs); this is most likely the only configuration you will ever need, as no further input is required.

- `bmh-example_slaac+dhcp+dns.yaml` - this configuration is identical to the `bmh-example_slaac+dhcp.yaml` above, with the exception of statically provided DNS servers; it can be useful in networks where only RAs are avaiable but with no DNS option.
     Requires:
 ...
 * The network configuration will only apply to the provisioned BMH and not yet on the inspection phase of the host to gather hardware information. This is due to a bug that will force the use of RA/DHCP with the default settings; a resolution is espected in the coming weeks.
...
- `single-cni`:
    - `cluster-v6-calico.yaml` - the cluster will use Calico as the sole CNI plugin in the cluster
    - `cluster-v6-canal.yaml` - the cluster will use Canal as the sole CNI plugin in the cluster
    - `cluster-v6-cilium.yaml` - the cluster will use Cilium as the sole CNI plugin in the cluster

- `multus-cni`:
   - `cluster-v6-calico_hostlocal.yaml` - the cluster will use Calico as the CNI plugin in combination with "host-local" as Multus IPAM for the second interface
   - `cluster-v6-calico_whereabouts.yaml` - the cluster will use Calico as the CNI plugin in combination with "whereabouts" as Multus IPAM for the second interface
   - `cluster-v6-canal_hostlocal.yaml` - the cluster will use Canal as the CNI plugin in combination with "host-local" as Multus IPAM for the second interface
   - `cluster-v6-canal_whereabouts.yaml` - the cluster will use Canal as the CNI plugin in combination with "whereabouts" as Multus IPAM for the second interface
   - `cluster-v6-cilium_hostlocal.yaml` - the cluster will use Cilium as the CNI plugin in combination with "host-local" as Multus IPAM for the second interface
   - `cluster-v6-cilium_whereabouts.yaml` - the cluster will use Cilium as the CNI plugin in combination with "whereabouts" as Multus IPAM for the second interface
...
- `RKE2ControlPlane.spec.agentConfig.nodeName` - the hostname that will be set on the host
...
You can then test the new Kubernetes cluster by creating a Pod or a Deployment and a dual-stack Service. If you are using Multus, do not forget annotate your Pod or Deployment with the following, in order to reference the NetworkAttachmentDefinition:
